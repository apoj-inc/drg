...
check "versitygw_storage_mode" {
  assert {
    condition     = contains(["local_disk", "nfs"], try(local.service.storage.mode, ""))
    error_message = "VersityGW storage.mode must be local_disk or nfs."
  }
}

module "versitygw_instance" {
  ...
  data_disk_size             = try(local.service.storage.mode, "nfs") == "local_disk" ? tonumber(local.service.storage.local_disk.data_disk_gb) : null
  data_disk_datastore_id     = try(local.service.compute.data_disk_datastore, local.cluster.defaults.disk_datastore)
